...
export function initDatabase(): Database.Database {
  ...
  db.pragma('foreign_keys = ON') // CRITICAL: Enforces Cascade Deletes just like Entity Framework

  // 4. Generate the Complete Enterprise Schema
  // We wrap it in a transaction so if one table fails, they all fail (Safety First)
  db.exec(`
    BEGIN TRANSACTION;
...
// ============================================================================
// 📊 REPORTING & DASHBOARD QUERIES (Better-SQLite3 Synchronous Mode)
// ============================================================================

export function getDashboardMetrics() {
  const database = getDb()

  // 1. Today's Cash Sales (localtime modifier ensures it resets at midnight locally)
  ...
  // 3. Pending Credit / Debt (Status != 0 means Unpaid or PartiallyPaid)
...
export function getRecentTransactions(limit: number = 5) {
  ...
  // Fetch the latest transactions to show on the dashboard live feed
  return database
    .prepare(
      `
...
// Fetch historical transactions by Date and Search Term
...
// Fetch the line-items for a specific receipt
export function getReceiptDetails(receiptId: string) {
  const database = getDb()

  // We JOIN with the Products table so we get the actual Product Name!
  return database
    .prepare(
      `
